...
Get-ScheduledTask -ErrorAction SilentlyContinue |
  Where-Object { $_.TaskName -in @("NexumAltivonApi24h", "NexumAltivonApi24h5012") } |
...
Get-CimInstance Win32_Process -ErrorAction SilentlyContinue |
  Where-Object {
    ...
    $isServerRuntime = $path.StartsWith($BaseDirectory, [StringComparison]::OrdinalIgnoreCase) -or
      $command.IndexOf($BaseDirectory, [StringComparison]::OrdinalIgnoreCase) -ge 0 -or
      $path.IndexOf("\ProgramData\NexumAltivon_API_24H", [StringComparison]::OrdinalIgnoreCase) -ge 0 -or
      $command.IndexOf("\ProgramData\NexumAltivon_API_24H", [StringComparison]::OrdinalIgnoreCase) -ge 0 -or
      $path.IndexOf("\NexumAltivon_API_24H", [StringComparison]::OrdinalIgnoreCase) -ge 0 -or
      $command.IndexOf("\NexumAltivon_API_24H", [StringComparison]::OrdinalIgnoreCase) -ge 0

    $isServerRuntime -and (
      $_.Name -eq "NexumAltivon.API.exe" -or
      ($_.Name -match "^dotnet" -and $command -like "*NexumAltivon.API.dll*") -or
      ($_.Name -eq "powershell.exe" -and $command -like "*04-iniciar-api-24h.ps1*")
    )
  } |
...
if ($configText -match 'ASPNETCORE_URLS') {
  $configText = $configText -replace '\$env:ASPNETCORE_URLS\s*=\s*"[^"]*"', '$env:ASPNETCORE_URLS = "http://0.0.0.0:5012"'
} else {
  $configText += "`r`n`$env:ASPNETCORE_URLS = `"http://0.0.0.0:5012`"`r`n"
}
Set-Content -Path $ConfigTarget -Value $configText -Encoding UTF8

New-NetFirewallRule -DisplayName "Nexum Altivon API Comercial 5012" -Direction Inbound -Action Allow -Protocol TCP -LocalPort 5012 -ErrorAction SilentlyContinue | Out-Null
...
$apacheExe = "Y:\xampp\apache\bin\httpd.exe"
$apacheConfig = "Y:\xampp\apache\conf\httpd.conf"
$apacheServiceName = "Apache2.4"
if (-not (Test-Path $apacheExe) -or -not (Test-Path $apacheConfig)) {
  throw "Apache oficial do servidor nao encontrado em Y:\xampp\apache."
}

& $apacheExe -t -f $apacheConfig
if ($LASTEXITCODE -ne 0) {
  throw "Configuracao do Apache em Y: invalida."
}

$apacheService = Get-CimInstance Win32_Service -Filter "Name='$apacheServiceName'" -ErrorAction SilentlyContinue
$expectedApachePath = [System.IO.Path]::GetFullPath($apacheExe)
$serviceUsesY = $apacheService -and ([string]$apacheService.PathName).IndexOf($expectedApachePath, [StringComparison]::OrdinalIgnoreCase) -ge 0

if ($apacheService -and -not $serviceUsesY) {
...
if (-not $apacheService) {
  & $apacheExe -k install -n $apacheServiceName
  if ($LASTEXITCODE -ne 0) {
    throw "Falha ao instalar o Apache oficial do Y: como servico."
  }
}

Set-Service -Name $apacheServiceName -StartupType Automatic
Restart-Service -Name $apacheServiceName -Force
New-NetFirewallRule -DisplayName "Nexum Altivon HTTP 80" -Direction Inbound -Action Allow -Protocol TCP -LocalPort 80 -ErrorAction SilentlyContinue | Out-Null
New-NetFirewallRule -DisplayName "Nexum Altivon HTTPS 443" -Direction Inbound -Action Allow -Protocol TCP -LocalPort 443 -ErrorAction SilentlyContinue | Out-Null

$siteHealthy = $false
for ($attempt = 1; $attempt -le 15; $attempt++) {
  Start-Sleep -Seconds 2
  try {
    $siteResponse = Invoke-WebRequest -UseBasicParsing -Uri "http://127.0.0.1/" -Headers @{ Host = "nexumaltivon.com" } -TimeoutSec 5
    if ($siteResponse.StatusCode -eq 200) {
      $siteHealthy = $true
      break
    }
  } catch {
    Write-Host "Aguardando site: tentativa $attempt/15"
  }
}

if (-not $siteHealthy) {
  throw "Apache iniciou, mas a home nao respondeu em http://127.0.0.1/."
}
...
Write-Host "Home via Apache Y: OK"
