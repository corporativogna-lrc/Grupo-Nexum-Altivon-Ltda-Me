...
$ApiCommand = 'set ASPNETCORE_ENVIRONMENT=Development&& set ASPNETCORE_URLS=http://127.0.0.1:5012&& dotnet run --project "' + $ApiProject + '" --no-build --no-restore >> "' + $ApiLog + '" 2>&1'
$FrontendCommand = 'set BROWSER=none&& set PORT=3000&& set REACT_APP_BACKEND_URL=http://127.0.0.1:5012&& npm start >> "' + $FrontendLog + '" 2>&1'
...
Write-Host "API: http://127.0.0.1:5012"
