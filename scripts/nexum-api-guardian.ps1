...
$PublishedApiRuntimeConfig = Join-Path $RunDir "api-local\NexumAltivon.API.runtimeconfig.json"
...
function Start-ApiProcess {
  ...
  if ((Test-Path $PublishedApiDll) -and (Test-Path $PublishedApiRuntimeConfig)) {
