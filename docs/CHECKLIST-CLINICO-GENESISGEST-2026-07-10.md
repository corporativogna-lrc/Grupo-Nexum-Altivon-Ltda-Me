...
| Dropshipping | CRUD operacional incorporado ao modulo de Integracoes | `/api/dropshipping/canais*`, MySQL `dropshipping_config`, diagnosticos e roteamento existente | Parcial avancado | Em 2026-07-14, o schema enum e os tipos Shopify/CJ foram corrigidos; CRUD autenticado confirmou bloqueio 424 sem conector, criacao 201, consulta persistida, atualizacao 200, concorrencia 409, exclusao 204, soft-delete no MySQL e limpeza sem residuos. O painel React lista e edita canais sem expor segredos. Falta implementar e homologar o ciclo externo de catalogo, cotacao, pedido, retorno e rastreamento para cada provedor contratado |
...
| Portal dinamico administravel | Parcial | Configuracoes institucionais ja usam API/MySQL, mas composicao visual, banners, logomarcas, imagens de lojas e tratamento de midia ainda dependem excessivamente do bundle. Implementar biblioteca de midia persistida, referencias versionadas por API, validacao de arquivo, dimensoes responsivas e administracao sem recompilar o frontend |
