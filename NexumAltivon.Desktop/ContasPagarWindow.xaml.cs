...
public partial class ContasPagarWindow : Window, INotifyPropertyChanged
{
    ...
    private async void ExportarPdf_Click(object sender, RoutedEventArgs e)
    {
        ...
        try
        {
            var result = await _api.GetContasPagarPdfAsync(_terminal, _terminal.DesktopAccessToken);
            if (!result.Success || result.Content is null || string.IsNullOrWhiteSpace(result.FileName))
            {
                Status = $"Relatório de contas a pagar não gerado. {result.Detail}";
                return;
            }

            var dialog = new SaveFileDialog
            {
                Title = "Salvar relatório de contas a pagar",
